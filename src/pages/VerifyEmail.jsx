...
const VerifyEmail = () => {
  ...
  const handlesubmit = async (e) => {
    e.preventDefault();
    const otpArray = inputRef.current.map(e => e.value.trim());
    ...
    <form onSubmit={handlesubmit} className='bg-slate-900 p-8 rounded-lg shadow-lg w-96 text-sm'>
      ...
      <div className='flex justify-between mb-8' onPaste={handlePaste}>
        {Array(6).fill(0).map((_,index)=>(
          ...
          className='w-12 h-12 bg-[#333A5C] text-white text-center text-xl rounded-md' ref={e=>inputRef.current[index] = e} onInput={(e)=>handleInput(e,index)} onKeyDown={(e)=>handleKeyDown(e,index)}/>
        ))}
      </div>
